...
/** API 모드: APIYI(OpenAI 호환) vs Google(Gemini SDK) */
type ApiMode = 'apiyi' | 'google';

/** API 설정 (키, 모드, baseUrl) */
const getApiConfig = (): { mode: ApiMode; apiKey: string; baseUrl: string } => {
  const apiyiKey = typeof localStorage !== 'undefined' ? localStorage.getItem('tubegen_apiyi_key') : null;
  console.log('[API Config] APIYI key:', apiyiKey ? `있음 (${apiyiKey.slice(0, 8)}...)` : '없음');

  if (apiyiKey && apiyiKey.length > 5) {
    console.log('[API Config] APIYI 모드 사용');
    return { mode: 'apiyi', apiKey: apiyiKey, baseUrl: 'https://api.apiyi.com/v1' };
  }

  const geminiKey = typeof localStorage !== 'undefined' ? localStorage.getItem('tubegen_gemini_key') : null;
  const envKey = typeof process !== 'undefined' ? (process as any).env?.GEMINI_API_KEY : null;
  const key = (geminiKey && geminiKey.length > 5) ? geminiKey : (envKey || '');
  console.log('[API Config] Gemini key:', geminiKey ? '있음' : '없음', 'env:', envKey ? '있음' : '없음');

  if (!key || key.length < 5) {
    throw new Error('API 키가 설정되지 않았습니다. 설정 페이지에서 APIYI 또는 Gemini API 키를 입력해주세요.');
  }
  console.log('[API Config] Google 모드 사용');
  return { mode: 'google', apiKey: key, baseUrl: '' };
};

/** APIYI용: OpenAI 호환 chat completions 호출 */
const chatCompletionViaApiyi = async (
  model: string,
  systemPrompt: string,
  userMessage: string,
  ...
  if (config.mode !== 'apiyi') throw new Error('APIYI 모드가 아닙니다.');
...
/** Google SDK용: API 키로 GoogleGenAI 인스턴스 생성 */
const getAI = () => {
  const config = getApiConfig();
  if (config.mode !== 'google') {
    throw new Error('Google SDK는 Google API 키로만 사용 가능합니다.');
  }
...
export const findTrendingTopics = async (category: string, usedTopics: string[]) => {
  const config = getApiConfig();
  console.log('[findTrendingTopics] 모드:', config.mode, '키:', config.apiKey.slice(0, 8));
  ...
  console.log('[findTrendingTopics] Google 경로 진입');
  return retryGeminiRequest("Trend Search", async () => {
    const ai = getAI();
    const prompt = getTrendSearchPrompt(category, usedTopics.join(", "));
...
const generateScriptSingle = async (
  ...
  const config = getApiConfig();
  console.log('[generateScript] 모드:', config.mode);
...
export const generateAudioForScene = async (text: string) => {
  ...
  if (config.mode === 'apiyi') {
    console.warn('[TTS] APIYI 모드에서는 ElevenLabs TTS를 사용해주세요.');
    return null;
  }
...
"Slow gentle zoom in. Character slightly nods with a warm smile, eyes blinking naturally. Subtle breathing motion. Background remains static. Maintain original art style consistency."`;
    ...
    if (config.mode === 'apiyi') {
      const motionPrompt = (await chatCompletionViaApiyi('gemini-2.5-flash', '', prompt)).trim() || '';
